...
class Down(nn.Module):
    def __init__(self, in_channels, out_channels, use_axial_attention=False):
        super().__init__()
        ...
        if use_axial_attention:
            self.axial_attention = AxialAttention(dim=out_channels, num_dimensions=2, heads=8, dim_heads=None, dim_index=1, sum_axial_out=True)

    def forward(self, x):
        x = self.maxpool_conv(x)
        if self.use_axial_attention:
            # 应用轴向注意力前，可能需要调整x的形状来匹配轴向注意力模块的期望输入
            x = self.axial_attention(x)
        return x

if __name__ == '__main__':
    inputa = torch.randn(1, 3, 224, 224)
    model =Down(3, 64, True)
    a = model(inputa)
    print(a.shape)